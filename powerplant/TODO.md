...
- `stores/useTasksStore.ts` `SEED_TASKS`: decide whether the daily tasks (Wandeling / Studeerblok / Rustmoment) should be a template the user can pick from, or generated by the (future) AI planning, or simply start empty. The current seed shows them on day-1 with hardcoded times — probably not what we want long term.
